...
describe('PermissionChecker', () => {
  it('should match project rules with glob path patterns', () => {
    const checker = new PermissionChecker('standard', '/project', {
      projectRules: [
        ...
        { tool: 'Write', path: 'dist/**', decision: 'deny' },
      ],
      globalRules: [],
    })

    expect(checker.check('Read', { file_path: 'src/index.ts' })).toBe('allow')
    expect(checker.check('Read', { file_path: 'secret/key.pem' })).not.toBe('allow')
    expect(checker.check('Write', { file_path: 'dist/bundle.js' })).toBe('deny')
  })
  ...
  it('should prioritize project rules over global rules', () => {
    const checker = new PermissionChecker('standard', '/project', {
      projectRules: [
        { tool: 'Write', path: '**', decision: 'allow' },
      ],
      globalRules: [
        { tool: 'Write', path: '**', decision: 'deny' },
      ],
    })

    expect(checker.check('Write', { file_path: 'anything.ts' })).toBe('allow')
  })
  ...
  it('should fall back to built-in defaults when no rule matches', () => {
    ...
    expect(checker.check('Read', { file_path: 'foo.ts' })).toBe('allow')
    expect(checker.check('Grep', {})).toBe('allow')
    // Built-in: write tools -> ask
    expect(checker.check('Write', { file_path: 'foo.ts' })).toBe('ask')
  ...
  it('should allow everything in relaxed mode (except critical)', () => {
    const checker = new PermissionChecker('relaxed', '/project', {
      projectRules: [{ tool: 'Write', path: '**', decision: 'deny' }],
      globalRules: [],
    })

    expect(checker.check('Write', { file_path: 'foo.ts' })).toBe('allow')
  ...
  it('should downgrade allow to ask in strict mode (except read-only)', () => {
    ...
    // Write allow -> downgraded to ask in strict
    expect(checker.check('Write', { file_path: 'foo.ts' })).toBe('ask')
  ...
  it('should match rules without path/command against all invocations', () => {
    ...
    expect(checker.check('WebFetch', { url: 'https://example.com' })).toBe('deny')
  })
  ...
  it('allows read-only tools by default', () => {
    ...
    expect(checker.check('Read', {})).toBe('allow')
