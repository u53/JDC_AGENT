...
用户希望为 JDC Context Engine 存入 5 分钟快照：5 分钟内同一会话/actor/model 对应的 Engine 内容不允许变化，并继续像现在一样拼到 system 中。这样 Anthropic 的多轮 prefix cache 才能在上一轮写入后，于下一轮继续命中历史消息断点。

## 目标

- 为 JDC Context Engine 注入内容增加 5 分钟短期快照。
- 5 分钟内命中同一项目、同一 actor、同一 mode、同一模型/协议/selection bucket 的请求时，复用完全相同的 rendered prompt，即使用户下一轮消息文本不同。
...
```text
projectRoot + actorKey + mode + modelFamilyKey + modelProfileKey + selectionKey
```
...
- `modelProfileKey`：模型能力 profile id + evidence strictness。不同 grounding/contract 文案不共享快照。
- `selectionKey`：actor 的 file scope 与 preferred fact count。影响 retrieval selection 的 bucket 不共享快照。
...
- 超过 5 分钟 TTL。
- `cwd`、actor、mode、model/protocol、model profile、selection bucket 任一字段改变。
...
- `session-context.test.ts`：同一主会话、不同 user turn、5 分钟内连续两次请求只 build 一次 context，system prompt 中 Engine 片段完全相同。
- `session-context.test.ts`：超过 5 分钟后重新 build，新的 rendered prompt 可以变化。
- `prompt-snapshot-cache.test.ts`：不同 actor、项目、mode、model/profile/selection bucket 不共享快照。
